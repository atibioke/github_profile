...
const Card = () => {
  ...
  const getProfile = async () => {
    try {
      ...
      const currentSubmit = jsonData.items.filter((item)=> new Date(item.created_at).toLocaleString("en-US", { day : '2-digit'}) < 30)
            ...
            <div className="container" key={name}>
              ...
              <div  className="text-container">
               ...
               <p className="stars-container">Stars: {stargazers_count}</p>
                ...
                <p className="submitted">Submitted { new Date(created_at).toLocaleString("en-US", { day : '2-digit'})} days ago by {name}</p>
               </div>
              </div>
            </div>
          </>
